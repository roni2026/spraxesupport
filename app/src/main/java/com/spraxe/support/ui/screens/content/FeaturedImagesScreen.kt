...
@Composable
fun FeaturedImagesScreen(viewModel: FeaturedImagesViewModel = viewModel()) {
    Box(modifier = Modifier.fillMaxSize()) {
        if (viewModel.isLoading) {
            LoadingIndicator()
        } else if (viewModel.images.isEmpty()) {
            EmptyState("No hero banners yet. Tap + to add one.")
        } else {
                    ...
                    Card(modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp)) {
                        Row(modifier = Modifier.padding(16.dp).fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                            Column(modifier = Modifier.weight(1f)) {
                                Text(image.title ?: "Untitled banner", style = MaterialTheme.typography.titleMedium)
                                Text(image.description ?: "", style = MaterialTheme.typography.bodyMedium)
                            }
                            IconButton(onClick = { viewModel.startEdit(image) }) { Icon(Icons.Filled.Edit, contentDescription = "Edit") }
                            IconButton(onClick = { viewModel.delete(image) }) { Icon(Icons.Filled.Delete, contentDescription = "Delete") }
                        }
                    }
                }
            }
    ...
    if (viewModel.showForm) {
        AlertDialog(
            ...
            title = { Text(if (viewModel.editing == null) "New hero banner" else "Edit hero banner") },
            text = {
                Column {
